export default async function handler(req, res) {
  ...
  try {
    // Use exact coordinates passed in - no rounding that could shift location
    const latF = parseFloat(lat);
    const lonF = parseFloat(lon);
    const url = `https://api.met.no/weatherapi/locationforecast/2.0/compact?lat=${latF}&lon=${lonF}`;
